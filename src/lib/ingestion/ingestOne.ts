import type { LegalDocumentMetadata, LegalSourceAdapter, RawDocumentRef } from "@/lib/ingestion/types";
...
export async function ingestOneDocument(
  ...
  return ingestExtractedDocument(sourceId, metadata, content.text);
}

/**
 * Same dedup -> chunk -> embed -> store pipeline as ingestOneDocument, minus
 * the "fetch from an external adapter" step — for callers that already have
 * the raw text in hand (e.g. a user-uploaded PDF, see app/api/documents).
 */
export async function ingestExtractedDocument(
  ...
  const chunks = enforceMaxChunkSize(chooseChunker(metadata.sourceType)(rawText));
